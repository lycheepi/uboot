...
#define BSP_VERSION		 	"iW-PRFHZ-SC-01-R2.0-REL1.0-Linux4.9.51-PATCH005"
...
#define COMMON_CONFIG_EXTRA_ENV_SETTINGS \
	...
	"loadaddr=0x80280000\0"  \
               "ethaddr=00:01:02:03:04:05\0" \
				"eth1addr=1A:2B:3C:4D:5E:6F\0" \
                "kernel=Image\0" \
                "bootargs_base=console=ttyLP4,115200 earlycon=lpuart32,0x5a0a0000,115200\0" \
...
#define CONFIG_EXTRA_ENV_SETTINGS_SSD               \
	...
	"bootargs_ssd=setenv bootargs ${bootargs_base} " \
        ...
        "loadhdp=fatload mmc 1 ${hdp_addr} ${hdp_file}\0" \
        "bootcmd_ssd=run loadhdp; hdp load ${hdp_addr};run bootargs_ssd;mmc dev 1;" \
...
#define CONFIG_EXTRA_ENV_SETTINGS_MSD               \
         ...
         "loadhdp=fatload mmc 2 ${hdp_addr} ${hdp_file}\0" \
         "bootcmd_msd=run loadhdp; hdp load ${hdp_addr};run bootargs_msd;mmc dev 2;" \
...
#define CONFIG_EXTRA_ENV_SETTINGS_SATA               \
			 ...
			 "loadhdp=fatload scsi 0 ${hdp_addr} ${hdp_file}\0" \
		         "bootcmd_sata=run loadhdp; hdp load ${hdp_addr};run bootargs_sata;scsi scan;scsi dev 0;" \
...
#define CONFIG_EXTRA_ENV_SETTINGS_EMMC               \
        ...
        "bootargs_mmc=setenv bootargs ${bootargs_base} " \
                        "root=/dev/mmcblk0p2 rootwait rootfstype=ext4 rw \0" \
	"loadhdp=fatload mmc 0 ${hdp_addr} ${hdp_file}\0" \
        "bootcmd_mmc=run loadhdp; hdp load ${hdp_addr};run bootargs_mmc;mmc dev 0;" \
                        "fatload mmc 0 ${loadaddr} ${kernel};fatload mmc 0 ${fdt_addr} ${fdt_file};booti ${loadaddr} - ${fdt_addr}\0" \
        "bootcmd=run bootcmd_mmc\0" \
        "\0"
